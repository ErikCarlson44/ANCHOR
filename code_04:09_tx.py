...
def pack_telemetry_binary(lat, lon, hdg, spd, bat, obs_list):
    """
    Fixed 10-byte LoRa uplink. Always exactly 2 obstacle slots (nearest by radius).

    Layout:
      [0-1]  float16  lat          IEEE 754 half-precision
      [2-3]  float16  lon          IEEE 754 half-precision
      [4]    uint8    hdg          fixed-point: round(hdg * 255/359),  0-359 deg -> 0-255
      [5]    uint8    obs[0].r     fixed-point: min(255, round(dist_m))
      [6]    int8     obs[0].angle fixed-point: clamp(angle_deg, -128, 127)
      [7]    uint8    obs[1].r     fixed-point: min(255, round(dist_m))
      [8]    int8     obs[1].angle fixed-point: clamp(angle_deg, -128, 127)
      [9]    uint8    spd          round(knots), 0-255
      [10]   uint8    bat          0-100
    """
    # Sort valid obs by radius ascending, take closest 2, pad with nulls if fewer
    valid = []
    for o in obs_list:
        if isinstance(o, (list, tuple)) and len(o) >= 2:
            r = safe_json_float(o[0], 0.0)
            a = safe_json_float(o[1], 0.0)
            if r > 0:
                valid.append((r, a))
    valid.sort(key=lambda x: x[0])
    while len(valid) < 2:
        valid.append((0.0, 0.0))  # null/empty slot

    lat_f = safe_json_float(lat, DEFAULT_LAT)
    lon_f = safe_json_float(lon, DEFAULT_LON)
    hdg_f = safe_json_float(hdg, 0.0) % 360.0
    spd_f = safe_json_float(spd, 0.0)
    bat_i = int(max(0, min(100, safe_json_float(bat, 100.0))))

    hdg_b = int(round(hdg_f * 255.0 / 359.0)) & 0xFF
    spd_b = min(255, int(round(spd_f)))
    bat_b = bat_i

    def encode_obs(r, a):
        r_b = min(255, int(round(r))) & 0xFF
        a_i = int(max(-128, min(127, round(a))))
        a_b = a_i if a_i >= 0 else a_i + 256
        return r_b, a_b & 0xFF

    r0, a0 = encode_obs(*valid[0])
    r1, a1 = encode_obs(*valid[1])
...
# =============================================================================
# LoRa RFM95W (adafruit_rfm9x) — raw binary packets
# =============================================================================
class LoraRFM95:
    """
    ...
    def send_binary(self, packet_bytes):
        """Send a 10-byte fixed binary telemetry packet."""
        dbg("LoRa TX {} bytes  freq {:.4f} MHz".format(
            len(packet_bytes), self.get_actual_freq()
        ))
        return self.rfm.send(bytes(packet_bytes))
            ...
            if (now - last_telemetry) >= telemetry_interval:
                ...
                if lora:
                    ...
                    if lora_tx_counter >= LORA_SEND_EVERY_N:
                        lora_tx_counter = 0
                        try:
                            lora.send_binary(pack_telemetry_binary(
                                lat, lon, heading, spd, 100, []
                            ))
        ...
        if (now - last_telemetry) >= telemetry_interval:
            ...
            if lora:
                ...
                if lora_tx_counter >= LORA_SEND_EVERY_N:
                    ...
                    try:
                        lora.send_binary(pack_telemetry_binary(
                            lat, lon, heading, spd, 100, obstacles[-15:]
                        ))
                        time.sleep(30) #for testing
                    except Exception as e:
                        dbg("LoRa TX:", e)
