...
def main():
    ...
    print(
        "LoRa {:.1f} MHz SF{} BW{}k CR4/{} CRC={}".format(
            LORA_FREQ_MHZ, LORA_SF, LORA_BW // 1000, LORA_CR, LORA_CRC
        )
    )
    ...
    while True:
        ...
        # --- LoRa -> UART (commands to sensor) ---
        try:
            pkt = rfm.receive(timeout=0.0)
        except Exception:
            pkt = None
        if pkt:
            handle_lora_rx(bytes(pkt))

        # --- I2C poll from sensor (latest telemetry JSON line) ---
        ...
        if DEBUG_HEARTBEAT_S > 0 and (now - last_hb) >= DEBUG_HEARTBEAT_S:
            ...
            try:
                while not i2c.try_lock():
                    pass
                try:
                    seen_addr = I2C_ADDR in i2c.scan()
                finally:
                    i2c.unlock()
            except Exception:
                seen_addr = False
            print(
                "HB i2c_bytes={} i2c_frames={} json_err={} lora_rx={} addr42={} rd_err={} wr_ok={} wr_err={}".format(
                    i2c_bytes_total,
                    i2c_frames_total,
                    i2c_json_err_total,
                    lora_rx_total,
